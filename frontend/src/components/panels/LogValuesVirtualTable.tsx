    ...
    <div className="flex flex-col border border-border" style={{ height: `${height}px` }}>
      <div className="flex bg-muted dark:bg-card border-b border-border" style={{ height: `${HEADER_HEIGHT}px` }}>
        {logs.map((log, index) => (
          <div
            key={index}
            className="flex-1 px-4 py-2 text-left font-semibold text-foreground border-r border-border last:border-r-0 overflow-hidden text-ellipsis whitespace-nowrap"
          >
            {log.name}
          </div>
        ))}
